...
async function testServerConnection() {
    ...
    try {
        ...
        updateConnectionStatus('disconnected', 'Server offline (using fallback simulation)');
        return false;
    } finally {
...
function updateDataTable(data) {
    ...
    sampledData.forEach((point, index) => {
        ...
        const growth = index > 0 ? 
            ((point.population - sampledData[index - 1].population) / sampledData[index - 1].population * 100).toFixed(2) + '%' : 
            '-';
