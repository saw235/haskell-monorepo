...
function updateKeyboardStatus() {
    if (keyboardEnabled) {
        ...
        keyboardStatus.textContent = 'Click here and enable checkbox to use arrow keys...';
...
function sendKeyboardInput(key, direction) {
    if (socket && isConnected && keyboardEnabled) {
        ...
        socket.send(message);
        addMessage(`[SENT] keyboard: ${key} (${direction}) at (${virtualPosition.x}, ${virtualPosition.y})`, 'sent');
    }
}
