...
import RpgRuleset.Core.Types (UserRole(..), Category(..), SystemId(..), Tag(..))
...
roleParser = option (eitherReader parseRole)
  ( long "role"
 <> short 'r'
 <> metavar "ROLE"
 <> value Player
...
formatParser = option (eitherReader parseFormat)
  ( long "format"
 <> short 'f'
 <> metavar "FORMAT"
 <> value TextFormat
...
queryOptionsParser = QueryOptions
  ...
  <*> optional (Category . T.pack <$> strOption
      ( long "category"
     <> short 'c'
     <> metavar "CATEGORY"
     <> help "Filter by category" ))
  <*> optional (SystemId . T.pack <$> strOption
      ( long "system"
     <> short 's'
     <> metavar "SYSTEM"
     <> help "Filter by system" ))
  <*> many (Tag . T.pack <$> strOption
      ( long "tag"
     <> short 't'
     <> metavar "TAG"
