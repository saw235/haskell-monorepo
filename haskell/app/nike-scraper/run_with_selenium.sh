...
# Script to run nike-scraper with Selenium server
...
# Find the nike-scraper binary in runfiles
SCRAPER_BIN=""
if [ -n "$RUNFILES_DIR" ]; then
    # Use RUNFILES_DIR if set
    SCRAPER_BIN="$RUNFILES_DIR/_main/haskell/app/nike-scraper/nike-scraper"
else
    # Try to find it relative to the script
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    SCRAPER_BIN="$SCRIPT_DIR.runfiles/_main/haskell/app/nike-scraper/nike-scraper"
fi
...
if [ ! -f "$SCRAPER_BIN" ]; then
    ...
    SCRAPER_BIN="$SCRIPT_DIR/nike-scraper"
fi

# Check if the binary exists
if [ ! -f "$SCRAPER_BIN" ]; then
    echo "Error: nike-scraper binary not found at $SCRAPER_BIN"
    ...
    if [ -n "$RUNFILES_DIR" ]; then
        find "$RUNFILES_DIR" -name "*nike*" 2>/dev/null || echo "No nike files found"
    else
        find "$SCRIPT_DIR.runfiles" -name "*nike*" 2>/dev/null || echo "No nike files found"
    fi
    exit 1
fi

# Run the nike-scraper binary with provided arguments
echo "Running nike-scraper with arguments: $*"
exec "$SCRAPER_BIN" "$@" 