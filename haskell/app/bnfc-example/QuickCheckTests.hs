...
prop_addZeroIdentity expr = 
  eval (EAdd expr (EInt 0)) === eval expr .&&.
  eval (EAdd (EInt 0) expr) === eval expr

-- Property: Multiplying by one is identity
prop_mulOneIdentity :: Exp -> Property
prop_mulOneIdentity expr = 
  eval (EMul expr (EInt 1)) === eval expr .&&.
  eval (EMul (EInt 1) expr) === eval expr

-- Property: Multiplying by zero gives zero
prop_mulZero :: Exp -> Property
prop_mulZero expr = 
  eval (EMul expr (EInt 0)) === 0 .&&.
  eval (EMul (EInt 0) expr) === 0
...
prop_parseBasicExpressions = conjoin
...
prop_operatorPrecedence = conjoin
