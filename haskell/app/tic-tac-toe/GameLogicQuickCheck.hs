...
import qualified Test.QuickCheck.Monadic as QC (monadicIO, run, assert)
...
prop_valid_moves_bounds board = 
  all (\(row, col) -> row >= 0 && row <= 2 && col >= 0 && col <= 2) 
...
prop_valid_moves_empty board = 
  all (\(row, col) -> board !! row !! col == Game.Empty) 
      (Game.validMoves board)
...
prop_boardSize = 
  let board = Game.emptyBoard in
    conjoin [length board === 3, conjoin (map ((=== 3) . length) board)]
  ...
  forAll genBoard3x3 $ \board ->
    forAll arbitrary $ \(ValidPosition (row, col)) ->
      forAll arbitrary $ \player ->
        let newBoard = Game.updateBoard board (row, col) player in
          conjoin [length newBoard === 3, conjoin (map ((=== 3) . length) newBoard)]
...
prop_boardUpdateTarget =
  forAll genBoard3x3 $ \board ->
    forAll arbitrary $ \(ValidPosition (row, col)) ->
      forAll arbitrary $ \player ->
        let newBoard = Game.updateBoard board (row, col) player in
          newBoard !! row !! col === Game.Filled player
...
prop_boardUpdatePreservesOthers =
  forAll genBoard3x3 $ \board ->
    forAll arbitrary $ \(ValidPosition (row, col)) ->
      forAll arbitrary $ \player ->
        let newBoard = Game.updateBoard board (row, col) player in
          conjoin [ newBoard !! r !! c === board !! r !! c
                  | r <- [0..2], c <- [0..2], (r, c) /= (row, col)]
...
prop_noDoubleWin =
  forAll genBoard3x3 $ \board ->
    let winner = Game.getWinner board in
      case winner of
        Just player ->
...
main = do
  ...
  results1 <- sequence [ runBoolTest "Empty board valid moves" prop_empty_board_valid_moves,
  ...
  results2 <- sequence [ runPropertyTest "Valid moves in bounds (Property)" prop_validMovesInBounds seed,
  ...
  results3 <- sequence [ runBoolTest "Winning board game over" prop_winning_board_game_over,
  ...
  results4 <- sequence [ runBoolTest "Valid move succeeds" prop_valid_move_succeeds,
  ...
  results6 <- sequence [ runBoolTest "Game engine valid move" prop_game_engine_valid_move,
    runBoolTest "Game engine invalid move" prop_game_engine_invalid_move ]
  
  -- Game State
  putStrLn "\n7. Testing game state..."
  results7 <- sequence [ runPropertyTest "Game terminates" prop_gameTerminates seed,
