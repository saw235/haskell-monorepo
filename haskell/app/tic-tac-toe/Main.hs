...
import Control.Monad.State (State, get, put, modify, runState, evalState)
...
hasWinningLine board player = 
    any (all (== Filled player)) board ||  -- Rows
    any (all (== Filled player)) (transpose board) ||  -- Columns
    all (== Filled player) [board !! i !! i | i <- [0..2]] ||  -- Diagonal
    all (== Filled player) [board !! i !! (2-i) | i <- [0..2]]  -- Anti-diagonal
