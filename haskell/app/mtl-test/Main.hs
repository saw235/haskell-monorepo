...
testReader = do
    config <- ask
    return $ "App: " <> appName config <> 
             ", Debug: " <> T.pack (show (debugMode config)) <>
             ", Max Retries: " <> T.pack (show (maxRetries config))
...
complexOperation input = do
    ...
    return $ T.concat
        [ "Input: ", input
        , ", Random: ", T.pack (show randomNum)
        , ", Result: ", result
        , ", Counter: ", T.pack (show (counter state))
        ]
