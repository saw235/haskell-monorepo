...
computeDeltas NeuralNetwork {..} activations expectedOutputs =
  let outputLayer = last layers
      ...
      outputDelta =
        V.zipWith
          ( \act exp ->
              (act - exp) * derivative (activation outputLayer) act
          )
          outputActivation
          expectedOutputs
        ...
        V.imap
          ( \i activationValue ->
              let weightedSum = V.sum (V.imap (\j delta -> (weights nextLayer V.! j V.! i) * delta) currentDelta)
               in weightedSum * derivative (activation currentLayer) activationValue
          )
          currentActivation
...
computeLayerWeightGradients Layer {..} inputActivation delta =
  V.map
    ( \neuronDelta ->
        V.map (\input -> input * neuronDelta) inputActivation
    )
    delta
