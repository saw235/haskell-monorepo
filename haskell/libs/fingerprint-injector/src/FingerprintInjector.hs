...
generateNavigatorScript navigator = T.unlines $
  [ "// Inject navigator properties" ] ++
...
generateScreenScript screen = T.unlines
  [ "// Inject screen properties"
  ...
  where
    orientationObj = T.concat
      [ "{ type: \"", screenOrientationType screen, "\""
      , ", angle: ", T.pack (show (screenOrientationAngle screen))
      , " }"
      ]

-- | Generate JavaScript to inject WebGL properties
generateWebGLScript :: WebGL -> Text
generateWebGLScript webgl = T.unlines
  [ "// Inject WebGL properties"
  , "(() => {"
...
generateCanvasScript canvas = T.unlines
  [ "// Inject canvas fingerprint"
...
defineProperty obj prop value = T.concat
