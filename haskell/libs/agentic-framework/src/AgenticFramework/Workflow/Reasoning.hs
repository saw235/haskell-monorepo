...
import AgenticFramework.Workflow.DSL (llmCall, getUserPrompt)
...
data PremiseType
  ...
  | Affirmation    -- ^ "P is true" (P)
  | Negation       -- ^ "Q is false" (¬Q)
  | Observation    -- ^ Raw observation from input
  deriving (Show, Eq, Generic)
...
data Validity
  ...
  | Invalid        -- ^ Logical fallacy detected
...
data InferenceRule
  ...
  | ModusPonensChain -- ^ P → Q, Q → R, P ⊢ R (hypothetical syllogism + MP)
  | DisjunctiveSyllogism -- ^ P ∨ Q, ¬P ⊢ Q
  | NoInference    -- ^ No valid inference possible
  deriving (Show, Eq, Generic)
...
data ReasoningResult = ReasoningResult
  { resObservations :: Text           -- ^ Original observations
  , resArgument :: Argument           -- ^ The formal argument
  , resExplanation :: Text            -- ^ Natural language explanation
  , resIsSound :: Bool                -- ^ Soundness: premises are actually true
  , resIsValid :: Bool                -- ^ Validity: conclusion follows from premises
  }
  deriving (Show, Eq, Generic)
...
formalReasoningPrompt = T.unlines
...
parseReasoningResult observations response =
  let lines' = T.lines response
      ...
      -- Extract sections
      findLine prefix = T.strip . T.drop (T.length prefix) <$>
  ...
  in case (inferenceRule, conclusion, validity) of
       (Just rule, Just conc, Just val) -> Just $ ReasoningResult
...
extractPremises lines' =
  ...
  where
    isPremiseLine l = T.isPrefixOf "P1" l || T.isPrefixOf "P2" l ||
                      T.isPrefixOf "PREMISE" l

-- | Parse a single premise line
parsePremise :: Int -> Text -> Premise
parsePremise n line = Premise
  { premiseId = n
