...
data ExecutionLog = ExecutionLog
  { -- | Unique log identifier
    ...
    execLogEntries :: Seq LogEntry,
    -- | When execution started
    execLogStartTime :: UTCTime,
    -- | When execution ended (if completed)
    execLogEndTime :: Maybe UTCTime
  }
  deriving (Show, Eq)
...
instance FromJSON ExecutionLog where
  parseJSON = Aeson.withObject "ExecutionLog" $ \v ->
    ExecutionLog
      <$> v Aeson..: "id"
      <*> v Aeson..: "agent_id"
      <*> v Aeson..: "entries"
...
logAgentReasoning ::
  ...
  Text ->
  -- | Reasoning type (e.g., "planning", "reflection", "tool_selection")
  Text ->
  -- | Optional context or related tool
  Maybe Text ->
  m ()
