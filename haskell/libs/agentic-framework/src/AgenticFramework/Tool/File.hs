...
import System.Directory (listDirectory, doesFileExist, doesDirectoryExist, getFileSize, getModificationTime)
  ...
  , toolExecute = \(ToolInput input) -> do
      case extractPath input of
        ...
        Just filePath -> do
          ...
          if not exists
            then return $ Right $ ToolOutput $ object
                   [ "exists" .= False
                   , "is_file" .= False
                   , "is_directory" .= False
                   , "size" .= (0 :: Int)
              ...
              else return $ Right $ ToolOutput $ object
                     [ "exists" .= True
                     , "is_file" .= False
                     , "is_directory" .= True
                     , "size" .= (0 :: Int)
