...
data TokenizerError
  = UnsupportedModel ModelName      -- ^ Model not supported by tokenizer
  | TokenizationFailed Text         -- ^ General tokenization failure
  | FFIError Text                   -- ^ FFI call failed
...
foreign import ccall unsafe "tokenizer_count_tokens"
  c_tokenizer_count_tokens :: CString  -- ^ model name
                           -> CString  -- ^ text to tokenize
                           -> Ptr CInt -- ^ output token count
                           -> IO CInt  -- ^ error code
