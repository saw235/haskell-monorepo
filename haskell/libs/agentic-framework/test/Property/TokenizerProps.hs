...
spec = describe "Tokenizer Properties" $ do
  ...
  describe "Accuracy (SC-011)" $ do
    it "token count is reasonable for short texts" $ withMaxSuccess 10 $ property $
      \(ShortText text) ->
        let modelName = "gpt-4"
    ...
    it "token count is reasonable for long texts" $ withMaxSuccess 10 $ property $
      \(LongText text) ->
        let modelName = "gpt-4"
            charLen = T.length text
        in ioProperty $ do
  ...
  describe "Consistency" $ do
    it "same input produces same token count" $ withMaxSuccess 10 $ property $
      \(ShortText text) ->
        let modelName = "gpt-4"
        in ioProperty $ do
  ...
  describe "Proportionality" $ do
    it "significantly longer text has more tokens" $ withMaxSuccess 10 $ property $
      \(ShortText text1) ->
        let modelName = "gpt-4"
